...
const digitalCampaigns: DigitalCampaign[] = [
...
export default function Participation() {
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      // Program cards
      const progCards = programsRef.current?.querySelectorAll('.prog-card');
      if (progCards && progCards.length > 0) {
        gsap.set(progCards, { opacity: 1, y: 0 });
        gsap.from(progCards, {
          y: 50,
          opacity: 0,
          duration: 0.8,
      ...
      // Digital + conclusion cards
      const digCards = digitalRef.current?.querySelectorAll('.dig-card');
      if (digCards && digCards.length > 0) {
        gsap.set(digCards, { opacity: 1, y: 0 });
        gsap.from(digCards, {
          y: 50,
          opacity: 0,
          duration: 0.8,
    ...
    <section ref={sectionRef} className="relative w-full py-24 sm:py-32">
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <h2 className="font-display font-black text-3xl sm:text-4xl lg:text-5xl text-[#edffff] mb-4">
          Citizen
          <span className="text-[#5adbff]"> Engagement</span>
        </h2>
        ...
        <div className="grid lg:grid-cols-2 gap-6">
          ...
          {/* ── ENGAGEMENT PROGRAMS ── */}
          <div ref={programsRef} className="space-y-4">
            ...
            {programs.map((prog, i) => (
              <div
                key={i}
                className="prog-card glass-panel rounded-xl p-5 flex items-start gap-4 hover:border-[#edffff]/10 transition-all duration-300 group"
              >
                <div className="w-11 h-11 rounded-xl bg-[#edffff]/5 flex items-center justify-center flex-shrink-0 group-hover:bg-[#edffff]/10 transition-colors">
                  {React.createElement(prog.icon, { className: 'w-5 h-5 text-[#edffff]/70' })}
                </div>
                <div>
          ...
          <div ref={digitalRef} className="space-y-6">
            <div>
              <h3 className="font-display font-bold text-lg text-[#edffff] mb-4">
                Digital Campaigns
              </h3>
              <div className="flex flex-wrap gap-3">
                {digitalCampaigns.map((camp, i) => (
                  <div
                    key={i}
                    className="dig-card glass-panel rounded-xl px-5 py-3 flex items-center gap-3"
                  >
                    {React.createElement(camp.icon, { className: 'w-4 h-4 text-[#5adbff]' })}
            ...
            {/* Conclusion card */}
            <div className="dig-card relative overflow-hidden rounded-2xl p-8 bg-gradient-to-br from-[#5adbff]/10 to-[#c3e5ae]/5 border border-[#5adbff]/15">
